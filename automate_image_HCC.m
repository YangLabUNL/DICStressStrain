...
% images by the use of the matlab function cpcorr_2.m. 
...
% Keep in mind that CORRSIZE which you changed in cpcorr_2.m will limit your 
...
hold on
plot(grid_x,grid_y,'b+','LineWidth',2,'MarkerSize',10)            % plot the grid onto the image
hold off

% Start image correlation using cpcorr_2.m
...
for i=firstimage:(r-1)               % run through all images
    ...
    input_points_for(:,1)=reshape(input_points_x,[],1);         % we reshape the input points to one row of values since this is the shape cpcorr_2 will accept
    ...
    input_correl(:,:)=cpcorr_2(round(input_points_for), round(base_points_for), input, base);           % here we go and give all the markers and images to process to cpcorr_2.m which ic a function provided by the matlab image processing toolbox
    input_correl_x=input_correl(:,1);                                       % the results we get from cpcorr_2 for the x-direction
    input_correl_y=input_correl(:,2);                                       % the results we get from cpcorr_2 for the y-direction
    ...
    % Update base and input points for cpcorr_2.m
    base_points_x=grid_x;
    base_points_y=grid_y;
    ...
    hold on
    plot(grid_x,grid_y,'b+','LineWidth',2,'MarkerSize',10)                                % plot start position of raster
    plot(input_correl_x,input_correl_y,'r+','LineWidth',2,'MarkerSize',10)        % plot actual postition of raster
    hold off
    drawnow
