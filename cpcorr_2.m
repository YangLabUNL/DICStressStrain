function xymoving = cpcorr_2(varargin)
%cpcorr_2_2 Tune control point locations using cross-correlation. 
%   movingPointsAdjusted = cpcorr_2_2(movingPoints,fixedPoints,MOVING,FIXED) uses
...
%   in the fixed image.
%
%   cpcorr_2_2 returns the adjusted control points in movingPointsAdjusted, a
%   double matrix the same size as movingPoints.  If cpcorr_2_2 cannot
%   correlate a pairs of control points, movingPointsAdjusted will contain
%   the same coordinates as movingPoints for that pair.
%
%   cpcorr_2_2 will only move the position of a control point by up to 4
%   pixels.  Adjusted coordinates are accurate up to one tenth of a
%   pixel.  cpcorr_2_2 is designed to get subpixel accuracy from the image
%   content and coarse control point selection.
%
%   Note that the MOVING and FIXED images must have the same scale for
%   cpcorr_2_2 to be effective.
%
%   cpcorr_2_2 cannot adjust a point if any of the following occur:
...
%   Example
%   --------
%   This example uses cpcorr_2_2 to fine-tune control points selected in an
...
%       movingPointsAdjusted = cpcorr_2_2(movingPoints,fixedPoints,...
%                                 moving(:,:,1),fixed(:,:,1))
%
...
for icp = 1:ncp
    ...
    subpixel = true;
    [xpeak, ypeak, amplitude] = findpeak_2(norm_cross_corr,subpixel);
...
if size(xymoving_in,2) ~= 2 || size(xyfixed_in,2) ~= 2
    error(message('images:cpcorr_2_2:cpMatrixMustBeMby2'))
end

if size(xymoving_in,1) ~= size(xyfixed_in,1)
    error(message('images:cpcorr_2_2:needSameNumOfControlPoints'))
end

moving = varargin{3};
fixed = varargin{4};
if ~ismatrix(moving) || ~ismatrix(fixed)
    error(message('images:cpcorr_2_2:intensityImagesReq'))
end
...
if any(xymoving_in(:)<0.5) || any(xymoving_in(:,1)>size(moving,2)+0.5) || ...
   ...
   any(xyfixed_in(:,2)>size(fixed,1)+0.5)
    error(message('images:cpcorr_2_2:cpPointsMustBeInPixCoord'))
end
